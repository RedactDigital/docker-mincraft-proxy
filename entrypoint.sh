...
# Install velocity server if it doesn't exist
if [ ! -f $WORKDIR/$PROJECT*.jar ]; then
    ...
    # If the version is not set or the version is an empty string, we will use the latest version
    if [ -z "$VERSION" ]; then
...
# We are going to create an empty log file if it doesn't exist
